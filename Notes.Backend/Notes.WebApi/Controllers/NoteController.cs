...
namespace Notes.WebApi.Controllers
{
    ...
    /*
        * Эти атрибуты, используются в ASP.NET Core (часть платформы .NET для разработки веб-приложений 
          Microsoft) для определения свойств и характеристик API контроллеров и их методов. 

            * ApiVersion("1.0") и ApiVersion("2.0"): Атрибуты версионирования API, которые обозначают, 
              что контроллер поддерживает указанную версию API. Это помогает в управлении различными 
              версиями API.

            * Produces("application/json"): Этот атрибут указывает, что методы контроллера 
              по-умолчанию будут возвращать данные в формате JSON (JavaScript Object Notation). 
              Это стандартный формат для обмена данными между клиентом и сервером в современных 
              веб-приложениях.

            * Route("api/{version:apiVersion}/[controller]"): Этот атрибут определяет маршрут 
              или URL, по которому доступен контроллер. В этом случае URL будет содержать 
              'api', затем версию API, и название контроллера. [controller] будет автоматически 
              заменено на имя класса контроллера, а {version:apiVersion} - на версию API, которую 
              запросил клиент. Это позволяет реализовывать версионирование на уровне URL.

            * Например, если у вас есть контроллер с именем ProductsController и к нему обращаются 
              с версией 2.0, URL будет выглядеть примерно так: api/2.0/Products.

        Все эти атрибуты вместе помогают определить поведение и структуру контроллера в вашем API.
    */
    [ApiVersion("1.0")]
    [ApiVersion("2.0")]
    //[ApiVersionNeutral] - вызов версии если не указан
